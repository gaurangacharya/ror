    ...
    def search_with_sphinx(community_id:, search:, included_models:, includes:)
      ...
      if perform_numeric_search && numeric_search_match_listing_ids.empty?
        ...
        if input_latitude.present? && input_longitude.present?
          ...
          order ||= 'custom_rating DESC, geodist ASC'
          custom_rating_column = ", IF( `geodist` < 25 * 1609, `admin_rating`, 0) AS custom_rating"
        ...
        if search[:instant_booking].present?
          with[:instant_booking] = search[:instant_booking].to_i
        end

        if search[:guest_only].present?
          with[:guest_only] = search[:guest_only].to_i
        end

        if search[:affiliate].present?
          with[:author_affiliate] = search[:affiliate].map(&:to_i)
          if search[:affiliate] == [1]
            with[:affiliate_pricing] = [1]
          end
        end
        ...
        without_ids = nil
        with[:ajusted_listing_show_for_person_ids] = [ 0 ]
        if search[:referral_id].present?
          without_ids = ListingHideFromPerson.by_username(search[:referral_id]).map(&:listing_id)
          listing_show_for_person_ids = ListingShowForPerson.by_username(search[:referral_id]).map(&:id)
          with[:ajusted_listing_show_for_person_ids].concat(listing_show_for_person_ids)
        end

        final_search_params = {
          page: search[:page],
          select: "*, weight() as wght #{long_cond}#{custom_rating_column}",
        ...
        final_search_params[:without_ids] = without_ids if without_ids
