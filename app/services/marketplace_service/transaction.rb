  ...
  module Transaction
    ...
    module Query
      ...
      def transaction_with_conversation(transaction_id:, person_id: nil, community_id:)
        ...
        with_person = Maybe(person_id)
          .map { |p_id|
            [rel.where("starter_id = ? OR listings.author_id = ? OR (starter_id IN (SELECT id FROM people WHERE hotel_id = ?))", p_id, p_id, p_id)]
          }
          .or_else { [rel] }
          .first
    ...
    module Events
      ...
      def preauthorized(transaction, payment_type)
        ...
        AutomaticallyRejectPreauthorizedTransactionJob.set(priority: 8, wait_until: expire_at).perform_later(transaction[:id])
      ...
      def paid(transaction)
        ...
        DateTimeSlotService.accept_booking(transaction[:id])
      end
      ...
      def rejected(transaction)
        ...
        DateTimeSlotService.reject_booking(transaction[:id])
      end
      ...
      def setup_preauthorize_reminder(transaction_id, expire_at)
        ...
        if send_reminder
          TransactionPreauthorizedReminderJob.set(priority: 9, wait_until: reminder_at).perform_later(transaction_id)
        end
      end
