module ChargebeeRails
  class PlanSyncer
    ...
    def get_plans
      loop do
        item_price_list = retrieve_item_price_list
        @offset = item_price_list.next_offset
        cb_plans << item_price_list.flat_map(&:item_price)
    ...
    def retrieve_item_price_list
      options = { limit: 100 }
      options[:offset] = @offset if @offset.present?
      ChargeBee::ItemPrice.list(options)
    end
    ...
    def plan_params plan
      {
        ...
        chargebee_data: {
          ...
          item_family_id: plan.item_family_id,
