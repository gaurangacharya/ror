class StripeService::API::StripeApiWrapper
  ...
  class << self
    ...
    def membership_charge(community:, token:, coupon_id:, price:)
      coupon, can_use = Coupon.can_use?(community, coupon_id)

      if can_use
        price = (100 - coupon.value) / 100.0 * price
      end
      with_stripe_payment_config(community) do |payment_settings|
        Stripe::Charge.create({
          amount: price.to_i,
          description: "OwnOutdoors Membership - 1 year",
          currency: 'usd',
          capture: true,
          source: token
        })
    ...
    def perform_simple_transfer(community:, account_id:, amount_cents:, amount_currency:, charge_id:)
      with_stripe_payment_config(community) do |payment_settings|
        if charge_id
          Stripe::Transfer.create({
              amount: amount_cents,
      ...
      with_stripe_payment_config(community) do |payment_settings|
        ...
        when :direct
          ...
          if system_account.id != account_id
            ...
            Stripe::Refund.create({charge: charge_id}.merge(reason_data).merge(amount_data).merge(metadata: metadata))
          end
        end
      end
    end
    ...
    def get_platform_balance_txn(community:, balance_txn_id:)
      with_stripe_payment_config(community) do |payment_settings|
        Stripe::BalanceTransaction.retrieve(balance_txn_id)
      end
    end
    ...
    def get_external_charge(community:, charge_id:, stripe_account:)
      with_stripe_payment_config(community) do |payment_settings|
        Stripe::Charge.retrieve charge_id, stripe_account: stripe_account
      end
    end

    def capture_external_charge(community:, charge_id:, seller_id:, amount:)
      with_stripe_payment_config(community) do |payment_settings|
        charge = Stripe::Charge.retrieve charge_id, stripe_account: seller_id
        charge.capture({amount: amount}, {stripe_account: seller_id})
      end
    end

    def cancel_external_charge(community:, charge_id:, seller_id:)
      with_stripe_payment_config(community) do |payment_settings|
        charge = Stripe::Charge.retrieve charge_id, stripe_account: seller_id
        charge.refund
      end
