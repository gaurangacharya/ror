module TransactionService::Store::Transaction
  ...
  NewTransaction = EntityUtils.define_builder(
    ...
    [:booking_fields, :hash],
    [:deposit, :money],
    [:auto_confirm, :to_bool, :optional],
    [:referral_id, :string, :optional],
    [:referral_discount, :fixnum, :optional],
    [:person_white_label_id, :fixnum, :optional],
    [:add_ons_attributes, :array, :optional],
    [:tax_percent, :string, :optional],
    [:service_fee_percent, :string, :optional],
    [:listing_price, :money],
    [:fields_attributes, :array, :optional],
  )
  ...
  def upsert_shipping_address(community_id:, transaction_id:, addr:)
    Maybe(TransactionModel.where(id: transaction_id, community_id: community_id).first)
      ...
      .map { |a| a.update!(addr_fields(addr)) }
      .or_else { nil }
  end
  ...
  def update_booking_uuid(community_id:, transaction_id:, booking_uuid:)
    ...
    Maybe(TransactionModel.where(community_id: community_id, id: transaction_id).first)
      .each { |tx_model| tx_model.update(booking_uuid: UUIDUtils.raw(booking_uuid)) }
  ...
  def build_conversation(tx_model, tx_data)
    ...
    starter = ::Person.find(tx_data[:starter_id])
    if starter.hotel
      conversation.participations.build(
        person_id: starter.hotel_id,
        is_starter: true,
        is_read: true)
    end
    ...
    if is_booking?(tx_data)
      if tx_data[:booking_fields][:per_hour]
        ...
        tx_model.build_booking(
          listing_id: tx_model.listing_id,
