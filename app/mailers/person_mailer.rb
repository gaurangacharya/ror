...
class PersonMailer < ActionMailer::Base
  ...
  def conversation_status_changed(transaction, community, metadata = nil)
    ...
    set_up_layout_variables(recipient, community, @email_type, transaction)
  ...
  def new_message_notification(message, community)
    ...
    set_up_layout_variables(recipient, community, @email_type, message.conversation.tx)
    @person_white_label = message.conversation.tx.person_white_label
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      to_address = recipient.guest? ? [recipient.primary_email.address] : recipient.confirmed_notification_emails_to 
      @message = message
      service_name = @person_white_label&.full_name || community.full_name_with_separator(recipient.locale)
      sender_name = PersonViewUtils.person_display_name(message.sender, community)

      sending_params = {:to => to_address,
                        :subject => t("emails.new_message.you_have_a_new_message",
                                      sender_name: sender_name,
                                      service_name: service_name),
                        :from => community_specific_sender(community, message.conversation.tx)}

      premailer_mail(sending_params)
    end
  end

  def document_added_to_transaction(transaction, sender, community, document)
    @email_type =  "email_about_new_messages"
    @tx_model = transaction
    recipient = transaction.conversation.other_party(sender)
    set_up_layout_variables(recipient, community, @email_type, transaction)
    @person_white_label = transaction.person_white_label
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      to_address = recipient.guest? ? [recipient.primary_email.address] : recipient.confirmed_notification_emails_to 
      @message = I18n.t("document_email.body", name: sender.full_name, listing_title: transaction.listing_title)
      attachments[document.document_file_name] = File.read(document.document.path)
      sending_params = {:to => to_address,
                        :subject => I18n.t("document_email.subject", listing_title: transaction.listing_title),
                        :from => community_specific_sender(community, transaction)}
  ...
  def transaction_confirmed(transaction, community)
    @email_type =  "email_about_completed_transactions"
    @conversation = transaction
    recipient = transaction.seller
    set_up_layout_variables(transaction.seller, community, @email_type, transaction)
    @person_white_label = transaction.person_white_label
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community, transaction),
  ...
  def transaction_automatically_confirmed(transaction, community)
    @email_type =  "email_about_completed_transactions"
    @conversation = transaction
  ...
  def new_testimonial(testimonial, community)
    ...
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      ...
      @service_name = community.full_name_with_separator(recipient.locale)
      @name = PersonViewUtils.person_display_name(testimonial.author, community)
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
                     :subject => t("emails.new_testimonial.has_given_you_feedback_in_kassi", name: @name, service_name: @service_name))
    end
  end
  ...
  def confirm_reminder(transaction, recipient, community, days_to_cancel)
  ...
  def testimonial_reminder(transaction, recipient, community)
  ...
  def new_comment_to_own_listing_notification(comment, community)
    ...
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      @comment = comment
      author = PersonViewUtils.person_display_name(comment.author, community)
      service_name = community.full_name_with_separator(recipient.locale)
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
                     :subject => t("emails.new_comment.you_have_a_new_comment", author: author, service_name: service_name))
    end
  end
  ...
  def new_referral_signup(community, recipient, new_user)
    set_up_layout_variables(recipient, community)
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      @new_user = new_user
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
                     :subject => t("emails.new_referral_signup.subject"))
    end
  end

  def membership_cancelled(recipient, community)
    set_up_layout_variables(recipient, community)
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
                     :subject => t("emails.membership_cancelled.subject"))
    end
  end

  def membership_cancelled_admin(recipient, user, community)
    set_up_layout_variables(recipient, community)
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      @member_user = user
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
                     :subject => t("emails.membership_cancelled_admin.subject"))
    end
  end
  ...
  def email_confirmation(email, community)
    ...
    @service_name = @current_community.full_name(email.person.locale)
    with_locale(email.person.locale, community.locales.map(&:to_sym), community.id) do
      ...
      premailer_mail(:to => email.address,
                     :from => community_specific_sender(community),
                     :subject => t("devise.mailer.confirmation_instructions.subject", :service_name => @service_name),
  ...
  def welcome_email(person, community, regular_email=nil, test_email=false, referral_id=nil)
    @recipient = person
    recipient = person
    with_locale(recipient.locale, community.locales.map(&:to_sym), community.id) do
      ...
      @referral_id ||= @recipient.referral_id
      @referral_id ||= '(Not provided)'
      ...
      subject = if @recipient.has_admin_rights?(@current_community) && !@test_email
        ...
        t("emails.welcome_email.welcome_email_subject", :service_name => community.full_name(recipient.locale), :person => PersonViewUtils.person_display_name_for_type(person, "first_name_only"))
      end
      premailer_mail(:to => recipient.confirmed_notification_emails_to,
                     :from => community_specific_sender(community),
