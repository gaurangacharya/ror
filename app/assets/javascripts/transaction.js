...
(function(module, _) {
  ...
  function initiateTransactionScreen(options) {
    $('#transaction-agreement-read-more').click(function() { $('#transaction-agreement-content').lightbox_me({centered: true, zIndex: 1000000}); });

    $("#confirmed_availability").click(function(){
      if(this.checked) $(".avail-options").show(); else $(".avail-options").hide();
    })
    $.fn.datepicker.dates[options.locale] = options.localized_dates;
    var today = new Date();
    today = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 0, 0, 0, 0);
    picker = $("#dd").datepicker({autoclose: true, language: options.locale, startDate: today})
    new TimePicker("#tf", "#tt", picker, "ALLDAY");
    ...
    $(function(){
      ...
      $('#user_hotel_guest').change(function(){
        if ($(this).val() === '1') {
          $('#user_name_on_reservation').addClass('required');
        } else {
          $('#user_name_on_reservation').removeClass('required');
        }
      });
    });
  ...
  module.initiateTransactionScreen = initiateTransactionScreen;
