...
(function(module) {
  ...
  var setupPerDayOrNight = function(options, min_units) {
  ...
  */
  ...
  var setupPerHour = function(options) {
    ...
    var fixedHours = options.fixed_hours;
    ...
    $('#start_time').on('change', function() {
      ...
      endTime.find('option').each(function () {
        ...
        var availableExpression = endTimeIndex >= startTimeindex + options.min_hours * options.parts_per_hour;

        if (options.booking_duration) {
          availableExpression = (endTimeIndex >= startTimeindex) && (endTimeIndex - startTimeindex) == (options.booking_duration * options.parts_per_hour);
        }

        if (availableExpression && startTimeSlot === endTimeSlot &&
    ...
    var setUpSelectOptions = function(date, start, selectSelector) {
      ...
      for(var index in date_options) {
        ...
        if (!start && option.slot_end && !prevDisabled) {
          ...
          disabled = option.disabled || start && option.disabled_start ? ' disabled ' : '';
          blocked = option.disabled  || start && option.disabled_start ? 'true' : '';
        }
    ...
    var setEndTime = function(startTimeindex) {
      var endTimeIndex = startTimeindex + fixedHours * options.parts_per_hour;
      var dateOptions = options.options_for_select[currentDate];
      var endTime = currentDate + ' ' + dateOptions[endTimeIndex].value;
    ...
    if(dateInput.val()) { picker.datepicker('setDate', dateInput.val()); }
    validateForm();
  };

  var setupTxDateField = function(options) {
    $.fn.datepicker.dates[options.locale] = options.localized_dates;
    var today = dateAtBeginningOfDay(new Date());
    var pickerOptions = {
      autoclose: true,
    ...
    $fields = $('[data-tx-field-date]');
    var picker = $fields.datepicker(pickerOptions);
  };

  module.FromToDatePicker = {
    setupPerDayOrNight: setupPerDayOrNight,
    setupPerHour: setupPerHour,
    setupTxDateField: setupTxDateField,
  };
})(window.ST);
