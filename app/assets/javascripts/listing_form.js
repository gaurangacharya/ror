...
(function(module) {
  ...
  module.initialize_new_listing_form = function(
    ...
    var toggleCallForPrice = function() {
      if($("#listing_call_for_price").is(":checked") || $("#listing_restaurant").is(":checked")) {
    ...
    toggleCallForPrice();
    $(document).on("click", "#listing_call_for_price, #listing_restaurant", toggleCallForPrice);

    var toggleRestaurant = function() {
      if($("#listing_restaurant").is(":checked")) {
        $("#restaurant_price").attr('disabled', false);
        $(".restaurant-price-container").show();
      } else {
        $("#restaurant_price").attr('disabled', true);
        $(".restaurant-price-container").hide();
      }
    }
    toggleRestaurant();
    $(document).on("click", "#listing_restaurant", toggleRestaurant);
    ...
    module.listingForm = $(form_id).validate({
      ...
      rules: _.extend(numericRules, {
        ...
        "listing[price]": {
          required: function() {
            return !$("#listing_call_for_price").is(":checked");
          },
          money: true,
          minimum_price_required: function() {
            var listingPrice = parseFloat($('#listing_price').val());
            if (!isNaN(listingPrice) && listingPrice > 0) {
              return [minimum_price, subunit_to_unit]
            }
            return false;
          },
        },
