...
function add_validator_methods() {
  ...
  */
  ...
  $.validator.addMethod("phoneNumber", function(value, element) { return Inputmask.isValid(value, {alias: 'phone'}); }, "Phone number is invalid");
  $.validator.addMethod("websiteUrl", function(value, element) { return !$(".website-url").is(":visible") || url_regexp_check(value); }, "Website URL is invalid");
}
...
function initialize_signup_form(locale, username_in_use_message, invalid_username_message, email_in_use_message, invalid_invitation_code_message, name_required, invitation_required, referral_id_message) {
  ...
  $(form_id).validate({
    ...
    rules: {
      ...
      "person[referrer_id]": {remote: "/people/check_referral_id"}
    },
    messages: {
      ...
      "person[referrer_id]": { remote: referral_id_message }
    },
...
function initialize_update_account_info_form(locale, change_text, cancel_text, email_in_use_message, one_email_must_receive_notifications_message) {
  $('.account-new-email-link').click(
    function(event) {
      ...
      var content = $('.account-settings-hidden-email-form').html();
      var new_id = new Date().getTime();
      var regexp = new RegExp("new_email", "g")

      $('#account-settings-email-content-rows').append(content.replace(regexp, new_id));
    }
  );
  $(document).on('click', '.account-hide-new-email-link',
    function(event) {
      event.preventDefault();
      $(this).closest('.account-settings-email-row').remove();
...
function initialize_pending_consent_form(email_invalid_message, invitation_required, invalid_invitation_code_message, referral_id_message) {
  $('#help_invitation_code_link').click(function(link) {
    $('#help_invitation_code').lightbox_me({centered: true, zIndex: 1000000 });
  });
  $('#terms_link').click(function(link) {
  ...
  $("#form_phone_number").inputmask({alias: 'phone'});
  $("#form_is_vendor").change(function(){
    if($(this).val() == '1' || $(this).val() == '-1') $(".website-url").show(); else $(".website-url").hide();
  });
  $("#form_is_vendor").trigger("change")
  ...
  $('#pending_consent_form').validate({
    ...
    rules: {
      ...
      "form[phone_number]": {required: true, phoneNumber: true},
      "form[website_url]": { websiteUrl: true},
      "form[referrer_id]": { remote: "/people/check_referral_id" }
    },
    messages: {
      "form[email]": { remote: email_invalid_message },
      "form[invitation_code]": { remote: invalid_invitation_code_message },
      "form[referrer_id]": { remote: referral_id_message }
    }
  });
}
