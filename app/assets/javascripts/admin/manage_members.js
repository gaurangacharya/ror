...
window.ST.initializeManageMembers = function() {
  ...
  var togglePremium = function(url) {
    $.ajax({
      type: "PUT",
      url: url,
      dataType: "JSON",
  ...
  $(document).on("change", ".admin-members-premium-toggle", function(){
    var premium = this.checked;
    var row = $(this).parent().parent()[0];
    $('#admin-members-saving-posting-allowed').show();
    if(!premium) {
      if(confirm($(this).data("confirm-premium-off"))) {
        togglePremium($(this).data("premium-off-url"));
      } else {
        this.checked = true;
        $('#admin-members-saving-posting-allowed').hide();
      }
    ...
    if(premium) {
      if(confirm($(this).data("confirm-premium-on"))) {
        togglePremium($(this).data("premium-on-url"));
      } else {
        this.checked = false;
        $('#admin-members-saving-posting-allowed').hide();
      }
