...
class Admin::EmailsController < Admin::AdminBaseController
  ...
  def create
    ...
    if params[:test_email] == '1'
      CommunityMemberEmailSentJob.perform_later(@current_user.id, @current_user.id, @current_community.id, "[TEST] "+params[:email][:subject].to_s, content, params[:email][:locale])
      render body: t("admin.emails.new.test_sent"), layout: false
    else
      CreateMemberEmailBatchJob.perform_later(@current_user.id, @current_community.id, params[:email][:subject], content, params[:email][:locale], params[:email][:recipients])
