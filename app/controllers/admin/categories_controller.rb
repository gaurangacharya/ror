class Admin::CategoriesController < Admin::AdminBaseController
  ...
  def destroy_and_move
    ...
    if new_category
      # Move listings
      @category.own_and_subcategory_listings.update(:category_id => new_category.id)
  ...
  def category_params
    params.require(:category).slice(:parent_id, :translation_attributes, :sort_priority, :url, :basename, :quick_filter, :icon).permit!
  end
