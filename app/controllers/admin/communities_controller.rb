class Admin::CommunitiesController < Admin::AdminBaseController
  ...
  def test_welcome_member_email
    MailCarrier.deliver_later(PersonMailer.welcome_email(@current_user, @current_community, true, true, 'T0123456'))
    flash[:notice] = t("layouts.notifications.test_welcome_email_delivered_to", :email => @current_user.confirmed_notification_email_to)
    redirect_to edit_welcome_email_admin_community_path(@current_community)
  end
  ...
  def update_settings
    ...
    permitted_params = [
      ...
      :affiliate_percent,
      :membership_extend,
      :hotel_percent_platform,
      :hotel_percent_user,
      :hotel_percent_guest,
      :platform_percent_guest,
      :referral_discount,
      :self_service_price,
