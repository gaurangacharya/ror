...
class HomepageController < ApplicationController
  ...
  before_action :unset_person_popup, only: [:index]
  after_action :increase_session_person_popup, only: [:index]
  ...
  def index
    if @current_user&.is_hotel? || @current_user&.has_admin_rights?(@current_community) || session[:x_referral_id].present?
      if params[:instant_booking].blank?
        params[:instant_booking] = 'yes'
      end
    end
    ...
    @view_type = params[:'view-type'].blank? ? "map" : params[:'view-type']
    ...
    if FeatureFlagHelper.feature_enabled?(:searchpage_v1)
      ...
      search_result.on_success { |listings|
        listings.each {|l| l[:icon_name] = nil unless @allow_to_see_affiliates }
        ...
        elsif view_type_custom_list?
          render partial: 'custom_list', locals: { show_distance: location_in_use, viewport: viewport, shape_name_map: shape_name_map }
  ...
  def set_referral
    session[:x_referral_id] = params[:referral_id]
    person = person_by_session_x_referral
    unset_session_person_popup
    if person && person.referral_location && person.referral_location.has_coords?
      extra_params = "#{person.referral_location.bounding_box_params}&instant_booking=yes&unset_session_person_popup=1"
    else
      extra_params = "&unset_session_person_popup=1"
    end
    if person_white_label = person&.person_white_label
      if person_white_label.domain.present?
        uri = Rails.env.development? ? URI::HTTP : URI::HTTPS
        url = uri.build(
          host: person_white_label.domain,
          port: Rails.env.development? ? request.port : nil,
          path: search_with_locale_path,
          query: extra_params)
        return redirect_to url.to_s
      else
        redirect_path = white_label_search_path(username: person&.username)
      end
    else
      redirect_path = search_with_locale_path
    end
    redirect_to "#{redirect_path}?#{extra_params}"
  end

  def view_type_custom_list?
    params[:'view-type'] == 'custom-list'
  end
  helper_method :view_type_custom_list?
  ...
  def find_listings(params:, current_page:, listings_per_page:, filter_params:, includes:, location_search_in_use:, keyword_search_in_use:, relevant_search_fields:)
      if (@current_user&.premium_active? || @current_user&.is_affiliate? || @current_user&.is_hotel? || @current_user&.has_admin_rights?(@current_community))
        @allow_to_see_affiliates = true
        if params[:affiliate] == 'only'
          affiliate_search = [1]
        else
          affiliate_search = []
        end
    ...
    instant_booking = params[:instant_booking] == 'yes' ? 1 : (params[:instant_booking] == 'no' ? 0 : nil)
    referral_id = session[:x_referral_id].present? ? session[:x_referral_id].to_s.sub(/^R-/,'') : nil
    if @person_white_label
      referral_id = @person_white_label&.person&.username && @person_white_label&.person&.username;
    end
    guest_booking = referral_id ? nil : 0
  ...
  def location_search_params(params, keyword_search_in_use)
    ...
    center_point = if params[:lc].blank? && limit_search_distance && corners&.length == 4
