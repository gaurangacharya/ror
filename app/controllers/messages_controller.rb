class MessagesController < ApplicationController
  ...
  def is_participant?(person, conversation_id)
    Conversation.find(conversation_id).participant?(person) || is_tx_hotel?(person, conversation_id)
  end

  def is_tx_hotel?(person, conversation_id)
    tx = Conversation.find(conversation_id).tx
    tx && (tx.starter == person || tx.starter.hotel == person)
  end
end
