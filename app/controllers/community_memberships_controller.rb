  ...
  def give_consent
    ...
    check_result.and_then { |invitation_code, email_address, consent|
      ...
      @current_person.update({
        is_vendor: params[:form][:is_vendor] == '1' || params[:form][:is_vendor] == '-1',
        phone_number: params[:form][:phone_number],
        website_url: params[:form][:website_url],
        signup_charge_id: @signup_charge&.id,
        is_hotel: params[:form][:is_vendor] == '-1',
      })
      ...
      CommunityJoinedJob.perform_later(@current_user.id, @current_community.id)
      SendWelcomeEmail.set(priority: 5).perform_later(@current_user.id, @current_community.id)
