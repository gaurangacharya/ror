class PeopleController < Devise::RegistrationsController
  ...
  def new
    ...
    unless params[:profile_type].present?
      @extra_body_class = "signup-page"
      @plan_main = SignupPlan.find_by_code('head')
      @groups = SignupGroup.active.sorted
  ...
  def create
    ...
    error_redirect_path = domain + sign_up_path + "?profile_type=#{params[:profile_type]}"
    if params[:profile_type] == 'reseller' && params[:reseller_mode] == 'service' && params[:stripe_token].present?
      begin
        price = @current_community.self_service_price_cents
        @signup_charge = StripeService::API::StripeApiWrapper.membership_charge(community: @current_community.id, token: params[:stripe_token], coupon_id: params[:coupon], price: price)
    ...
    begin
      ActiveRecord::Base.transaction do
        ...
        @person.notice_coupon(@current_community.id, params[:coupon])
      end
      if @signup_charge
        # Delayed::Job.enqueue(MembershipPayoutJob.new(@current_community.id, @person.id))
      end
    rescue => e
    ...
    CommunityJoinedJob.perform_later(@person.id, @current_community.id) if @current_community
  ...
  def update
    ...
    begin
      ...
      if email_params = person_params[:emails_attributes].to_h.find do |key, values| key.to_i > 1_000_000_000 end&.last
        new_email_address = email_params[:address]
      end
      ...
      if target_user.update(person_params)
  ...
  def check_coupon
    ...
    coupon, can_use = Coupon.can_use?(@current_community.id, coupon_id)
    respond_to do |format|
      format.json { render :json => {value: coupon&.value} }
    end
  end

  def check_referral_id
    ref_id = (params[:person] || params[:form] || params)[:referrer_id]
    person = Person.where(community_id: @current_community.id, referral_id: ref_id).first
    respond_to do |format|
      format.json { render :json => ref_id.blank? || (person && (person.is_affiliate? || person.premium_active?)) }
    end
  end

  def check_referral_coupon
    ref_id = params[:code]
    person = Person.where(community_id: @current_community.id, referral_id: ref_id).first
    value = person && person.is_affiliate? ? @current_community.referral_discount : nil
    respond_to do |format|
  ...
  def profile_listings
    allow_iframe
    @presenter = ListingsPersonPresenter.new(@current_community, @current_user, params[:id], params)
  end

  def info
    @person = Person.find_by!(username: params[:id], community_id: @current_community.id, membership_status: true)
  ...
  def toggle_affiliate
    ...
    if @current_user && @current_user.has_admin_rights?(@current_community) && @person.is_vendor
      @person.toggle_affiliate
    end
  ...
  def new_person(initial_params, current_community)
    ...
    person.is_vendor = params[:is_vendor] == '1' || params[:is_vendor] == '-1'
    person.signup_charge_id = @signup_charge&.id
    person.is_hotel = params[:is_vendor] == '-1'
  ...
  def person_update_params(params)
    params.require(:person).permit(
        ...
        :referral_address,
        :customer_message_under_book,
        :embed_document,
        :pdf,
        :delete_pdf,
        :embed_document_button_color,
      )
  end
