class SettingsController < ApplicationController
  ...
  def membership
    @selected_left_navi_link = "membership"
    target_user = Person.find_by!(username: params[:person_id], community_id: @current_community.id)
    render :membership, locals: {target_user: target_user}
  end

  def cancel_membership
    target_user = Person.find_by!(username: params[:person_id], community_id: @current_community.id)
    target_user.update(membership_status: false)
    MembershipCancelledJob.perform_later(target_user.id, @current_community.id)
  ...
  def renew_membership
    ...
    if params[:stripe_token].present?
      begin
        @signup_charge = StripeService::API::StripeApiWrapper.membership_charge(community: @current_community.id, token: params[:stripe_token], coupon_id: params[:coupon])
        target_user.mark_as_member(@signup_charge.id, params[:referrer_id])
        target_user.notice_coupon(@current_community.id, params[:coupon])
  ...
  def chargebee_subscriptions
    @selected_left_navi_link = "chargebee-subscriptions"
    @plans = SignupPlan.plans
    @person = Person.find_by!(username: params[:person_id], community_id: @current_community.id)
  end
