class ListingsController < ApplicationController
  ...
  before_action :only => [ :edit, :edit_form_content, :update, :close, :follow, :unfollow, :featured, :copy_listing ] do |controller|
    controller.ensure_logged_in t("layouts.notifications.you_must_log_in_to_view_this_content")
  end

  before_action :only => [ :new, :new_form_content, :create, :copy_listing ] do |controller|
  ...
  before_action :only => [ :edit, :edit_form_content, :update, :copy_listing ] do |controller|
  ...
  before_action :is_authorized_to_post, :only => [ :new, :create, :copy_listing ]

  before_action :allow_iframe, :only => [:show]

  after_action :increase_session_person_popup, only: [:show]
  ...
  def create
    ...
    unless create_booking(listing_uuid, params[:listing][:booking_mode])
    ...
    ActiveRecord::Base.transaction do
      ...
      if @listing.save
        if params[:document_file].present?
          attach_document(@listing, params)
        end
        ...
        anchor = @listing.booking? ? 'manage-availability' : ''
  ...
  def update
    ...
    unless create_booking(@listing.uuid_object, params[:listing][:booking_mode])
      flash[:error] = t("listings.error.update_failed_to_connect_to_booking_service")
      return redirect_to edit_listing_path(@listing)
    end

    result = ListingFormViewUtils.build_listing_params(shape, @listing.uuid_object, params, @current_community, @listing)
    ...
    if update_successful
      ...
      flash[:notice] = update_flash(old_availability: old_availability, new_availability: @listing.availability.to_sym)
      ListingUpdatedJob.perform_later(@listing.id, @current_community.id)
      reprocess_missing_image_styles(@listing) if @listing.closed?
      anchor = @listing.booking? ? 'manage-availability' : ''
      redirect_to listing_path(@listing, anchor: anchor, listing_just_created: true)
    else
  ...
  def copy_listing
    @listing = Listing.find(params[:id])
    @new_listing = @listing.deep_clone
    redirect_to edit_listing_path(@new_listing)
  end
  ...
  def reprocess_missing_image_styles(listing)
    listing.listing_image_ids.each { |image_id|
      CreateSquareImagesJob.perform_later(image_id)
    }
  end
  ...
  def notify_about_new_listing
    ListingCreatedJob.perform_later(@listing.id, @current_community.id)
    if @current_community.follow_in_use?
      NotifyFollowersJob.set(wait: NotifyFollowersJob::DELAY).perform_later(@listing.id, @current_community.id)
    end
  ...
  def create_booking(listing_uuid, booking_mode)
    if availability_per_hour_enabled && booking_mode == 'hour'
      true
    elsif (booking_mode == 'day' || booking_mode == 'night')
      true
    else
      true
    end
  ...
  def attach_document(listing, params)
    ...
    if document.save
      listing.update(document_id: document.id)
    end
  end
