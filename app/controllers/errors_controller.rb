class ErrorsController < ActionController::Base
  ...
  def sentry_event_id
    request.env['sentry.error_event_id'] if event_id_present?(request.env['sentry.error_event_id'])
  end

  def can_notify_sentry
    defined?(Sentry) && Sentry.respond_to?(:capture_exception)
  end

  def use_sentry
    APP_CONFIG && APP_CONFIG.use_sentry
  end

  # Check if Sentry event ID is present and valid
  def event_id_present?(event_id)
    event_id.present? && event_id.is_a?(String) && !event_id.empty?
  end
