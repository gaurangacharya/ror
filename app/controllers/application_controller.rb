...
class ApplicationController < ActionController::Base
  ...
  def resolve_person_white_label
    request.env[:person_white_label]
  end

  # Before filter to get the current community
  def fetch_community
    @current_community = resolve_community()
    @person_white_label = resolve_person_white_label
    m_community = Maybe(@current_community)
  ...
  def fetch_community_membership
    if @current_user
      ...
      if (@current_community_membership && !date_equals?(@current_community_membership.last_page_load_date, Date.today))
        PageLoadedJob.perform_later(@current_community_membership.id, request.host)
      end
    end
  end
  ...
  def fetch_community_admin_status
    ...
    @current_admin = current_person if @is_current_community_admin
  end
  ...
  def select_app_layout
    if params[:iframe] || session[:iframe]
      session[:iframe] ||= request.referer || params[:iframe] || true
      @iframe_mode = true
      'application_iframe'
    else
      'application'
  ...
  helper_method :session_person_popup, :white_label_or_referral_person

  def session_person_popup(person)
    session["person-popup-#{person&.username}"]
  end

  def set_session_person_popup(person, data)
    session["person-popup-#{person&.username}"] = data
  end

  def increase_session_person_popup
    if person = white_label_or_referral_person
      set_session_person_popup(person, session_person_popup(person).to_i + 1)
    end
  end

  def unset_session_person_popup
    if person = white_label_or_referral_person
      set_session_person_popup(person, 0)
    end
  end

  def white_label_or_referral_person
    if @person_white_label.present?
      @person_white_label.person
    else
      person_by_session_x_referral
    end
  end

  def person_by_session_x_referral
    if session[:x_referral_id].present?
      username = session[:x_referral_id].to_s.sub(/^R-/,'')
      Person.find_by(username: username)
    end
  end

  def unset_person_popup
    if params[:unset_session_person_popup].present?
