module ListingFormViewUtils
  ...
  def validate(params:, shape:, unit:, valid_until_enabled: false)
    errors = []
    need_price = shape[:price_enabled] && params[:call_for_price] != '1' && params[:restaurant] != '1'

    errors << :price_required if need_price && params[:price].nil? 
    errors << :currency_required if need_price && params[:currency].blank?
  ...
  def normalize_price_params(listing_params)
    ...
    listing_params.inject({}) do |hash, (k, v)|
      ...
      when "visitor_price"
        hash.merge(:visitor_price_cents =>  MoneyUtil.parse_str_to_subunits(v, currency), :visitor_price_currency => currency)
      when "deposit"
  ...
  def build_listing_params(shape, listing_uuid, params, current_community, listing = nil)
    currency = listing&.currency || current_community.currency
  ...
  def create_listing_params(params)
    listing_params = params.except(:delivery_methods).merge(
      ...
      visitor_price_cents: params[:visitor_price_cents],
      visitor_price_currency: params[:visitor_price_currency],
