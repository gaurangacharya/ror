module PathHelpers
  ...
  def landing_page_path(community_id:, logged_in:, locale_param:, default_locale:, person_white_label:)
    if !!person_white_label
      person = person_white_label.person
      if person && person.referral_location && person.referral_location.has_coords?
        extra_params = "#{person.referral_location.bounding_box_params}&instant_booking=yes"
      else
        extra_params = ""
      end
      return "#{paths.search_with_locale_path(locale: locale_param || default_locale)}?#{extra_params}"
    end
