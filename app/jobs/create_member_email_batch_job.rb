class CreateMemberEmailBatchJob < ApplicationJob
  ...
  def perform(sender_id, community_id, subject, content, locale, mode)
    current_community = Community.where(id: community_id).first

    recipient_ids(mode, current_community).each do |recipient_id|
      CommunityMemberEmailSentJob.perform_later(sender_id, recipient_id, community_id, subject, content, locale)
    end
  end
