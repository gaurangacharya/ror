...
module CacheHelper
  ...
  def frontpage_fragment_cache(type, listing, &block)
    ...
    author = Person.find(listing.author.id) rescue nil
    cache([type, listings_i18n_digest, Listing.semi_premium_pricing?(listing), @current_user&.premium_pricing?(listing), @current_community, 
           listing, listing.author, author&.tripadvisor_location_id,
           MoneyRails::Configuration.no_cents_if_whole], :expires_in => FRAGMENT_CACHE_EXPIRE_TIME, &block)
  end
