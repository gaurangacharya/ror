...
module ApplicationHelper
  ...
  def one_line_for_html_safe_content(&)
    haml_concat capture_haml(&).gsub("\n", '').html_safe
  end
  ...
  def text_with_line_breaks_html_safe(&)
    haml_concat add_p_tags(capture_haml(&)).html_safe
  end

  # Changes line breaks to <br>-tags and transforms URLs to links
  def text_with_line_breaks(&)
    haml_concat add_links_and_br_tags(capture_haml(&)).html_safe
  end

  # Changes line breaks to <br>-tags and transforms URLs to links
  def text_with_line_breaks_for_email(&)
    haml_concat add_links_and_br_tags_for_email(capture_haml(&)).html_safe
  end
  ...
  def self.send_error_notification(message, error_class="Special Error", parameters={})
    if APP_CONFIG.use_sentry && defined?(Sentry) && Sentry.initialized?
      Sentry.with_scope do |scope|
        ...
        scope.set_context("custom_error", {
          message: message,
          parameters: parameters,
          environment: ENV.fetch('RAILS_ENV', nil)
        })
        # Create a custom exception for better tracking
        exception = StandardError.new(message)
        exception.set_backtrace($@) if $@
  ...
  def add_links(text)
    pattern = /[\.)]*$/
    text.gsub(/https?:\/\/\S+/) { |link_url| link_to(link_url.gsub(pattern,""), link_url.gsub(pattern,""), class: "truncated-link", target: "_blank", rel: "noopener") + link_url.match(pattern)[0]}
  end
  ...
  def admin_links_for(community)
    ...
    links += [
      ...
      {
        :topic => :manage,
        :text => t("admin.communities.premium_members.premium_members"),
        :icon_class => "ss-userprofile",
        :path => admin_premium_members_path,
        :name => "premium_members"
      },
      {
        :topic => :manage,
        :text => t("admin.coupons.coupons"),
        :icon_class => icon_class("order_types"),
        :path => admin_coupons_path,
        :name => "coupons"
      },
      {
        :topic => :manage,
        :text => t("admin.signup_plans.signup_plans"),
        :icon_class => "ss-moneybag",
        :path => admin_signup_plans_path,
        :name => "signup_plans"
      },
      {
        :topic => :manage,
        :text => t("admin.signup_groups.signup_groups"),
        :icon_class => "ss-moneybag",
        :path => admin_signup_groups_path,
        :name => "signup_groups"
      },
      {
        :topic => :manage,
        :text => t("admin.fingerprints.fingerprints"),
        :icon_class => "ss-cell",
        :path => admin_fingerprints_path,
        :name => "fingerprints"
      },
      {
        :topic => :manage,
      ...
      {
        :topic => :manage,
        :text => t("docusign.admin"),
        :icon_class => 'ss-bankcheck',
        :path => admin_docusign_templates_path,
        :name => "docusign"
      },
      {
        :topic => :configure,
  ...
  def settings_links_for(person, community=nil)
    ...
    if person.premium_active?
      links << {
        :id => "settings-tab-membership",
        :text => t("layouts.settings.membership"),
        :icon_class => "ss-userprofile",
        :path => membership_person_settings_path(person),
        :name => "membership"
      }
    end

    links << {
      :id => "settings-tab-chargebee-subscriptions",
      :text => t("layouts.settings.membership"),
      :icon_class => "ss-userprofile",
      :path => subscriptions_person_settings_path(person),
      :name => "chargebee-subscriptions"
    }
  ...
  def info_qr_code(person)
    qrcode = RQRCode::QRCode.new(info_person_url(person))
    svg = qrcode.as_svg(
      offset: 0,
      color: '000',
  ...
  def link_qr_code(url, viewbox: false)
  ...
  def guest_or_session_referral(person_white_label)
    session[:x_referral_id] || person_white_label&.person&.username || 'GUEST'
  end

  def tripadvisor_info_badge(user_id, hide_image=false, remove_url=false)
    person = Person.find(user_id)
    return '' unless person.tripadvisor_location_id.present?

    info = TripadvisorService.new(location_id: person.tripadvisor_location_id, logger: Rails.logger).info
    return '' unless info

    return '' unless info['web_url']

    render partial: 'tripadvisor/badge', locals: {ta_info: info, hide_image: hide_image, remove_url: remove_url}
  end
end
