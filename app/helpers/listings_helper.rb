module ListingsHelper
  ...
  def document_template_select_options(document_id)
    shared_templates = Document.by_community(@current_community.id).shared_templates.ordered
    own_templates = Document.by_author(@current_user.id).templates
    all_templates = (own_templates + shared_templates).uniq
    options_from_collection_for_select(all_templates, 'id', 'title_with_type', document_id)
  end

  def obfuscate_email(email)
    ObfuscateEmail.new(email)
  end
