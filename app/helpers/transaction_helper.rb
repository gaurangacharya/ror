module TransactionHelper
    ...
    statuses = if conversation.listing && !conversation.status.eql?("free")
      status_hash = {
        ...
        pending: ->() { {
          author: [
            status_info(
              t("conversations.status.waiting_payment_from_requester", requester_name: conversation.other_party(@current_user).name(conversation.community)),
              icon_classes: icon_for("pending")
            )
          ],
          starter: [
            status_info(
              t("conversations.status.waiting_payment_from_you"),
              icon_classes: icon_for("pending")
            ),
          ]
        } },
  ...
  def feedback_status(conversation)
    if conversation.author.reviews_disabled?
      nil
    elsif conversation.has_feedback_from?(@current_user)
  ...
  def working_time_slots_for_transaction(txn)
    date = txn.booking.start_time.to_date
    txn.listing.working_time_slots.by_date(date).map do |slot|
      slot.display_working_time_slot
    end
  end

  def booked_time_slots_for_transaction(txn)
    date = txn.booking.start_time.to_date
    txn.listing.bookings_per_hour.by_date(date).map do |booking|
      booking.display_booking
    end
  end

  def transaction_schedule_data(transaction_model)
    working_slots = working_time_slots_for_transaction(transaction_model)
    booked_slots = booked_time_slots_for_transaction(transaction_model)
    min_working_top = working_slots.map{|value| value[:top] }.min.to_i
    min_booked_top = booked_slots.map{|value| value[:top] }.min.to_i
    min_top = [min_working_top, min_booked_top].min
    max_working_bottom = working_slots.map{|value| value[:top] + value[:height] }.max.to_i
    max_booked_bottom = booked_slots.map{|value| value[:top] + value[:height] }.max.to_i
    max_bottom = [max_working_bottom, max_booked_bottom].max

    top = [min_top - AvailabilityMask::HOUR_HEIGHT, 0].max
    {
      height: [max_bottom - top + AvailabilityMask::HOUR_HEIGHT, 48 * AvailabilityMask::HOUR_HEIGHT].min,
      top: top,
      booked: booked_slots,
