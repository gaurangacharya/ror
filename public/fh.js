(function() {
    ...
    var composeUrl = (function() {
        var encodeUriQuery = function(val, pctEncodeSpaces) {
            return encodeURIComponent(val).replace(/%40/gi, '@').replace(/%3A/gi, ':').replace(/%24/g, '$').replace(/%2C/gi, ',').replace(/%2F/gi, '/').replace(/%20/g, (pctEncodeSpaces ? '%20' : '+'));
        };
    ...
    var appendPropertiesToUrl = (function() {
        ...
        var linkOptimizely = wrap(function() {
            try {
    ...
    var createCartframe = function() {
        ...
        container.innerHTML = '<iframe id="' + CARTFRAME_EMBED_ID + '" name="' + name + '" frameborder=0 border=0 src="' + src + '" style="display: none;" title=""></iframe>';
        createStyles();
    ...
    var notifyCartframe = function(type) {
        if (!createdCartframe) {
            return;
        }
        var iframe = document.getElementById(CARTFRAME_EMBED_ID);
        if (!iframe) {
            return;
        }
        var message = window.JSON.stringify({
            type: type
        });
    ...
    var createLightframe = function() {
        ...
        container.innerHTML = '<div id="fareharbor-lightframe-shade"></div>' + '<div id="fareharbor-lightframe-loading"></div>';
        document.body.insertBefore(container, document.body.firstChild);
    };
    ...
    var createEmbedHtml = function(src) {
        ...
        var iframeHtml = '<iframe id="' + embedId + '" name="' + name + '" frameborder=0 border=0 width="100%" src="' + src + '"></iframe>';
        if (isWrapped) {
    ...
    window.addEventListener('pointerdown', function onPointerDown(evt) {
    ...
    handlers['fareharbor.cart.changed'] = function() {
        notifyCartframe('fareharbor.cart.changed');
    };
    ...
    window.addEventListener('focus', function() {
        notifyCartframe('fareharbor.window.focused');
    });
