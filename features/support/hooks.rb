...
After do |scenario|
  if(scenario.failed?)
    ...
    scenario_name = scenario.name.gsub(/[^a-zA-Z0-9\-_]/, '_').gsub(/_+/, '_')
    filename = "tmp/screenshots/FAILED_#{scenario_name}_#{timestamp}.png"
    save_screenshot(filename)
    puts "❌ Failure screenshot saved: #{filename}"
