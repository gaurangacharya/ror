...
Given /^the price of that listing is (\d+)\.(\d+) (EUR|USD)(?: per (.*?))?$/ do |price, price_decimal, currency, price_per|
  unit_type = if ["piece", "hour", "day", "night", "week", "month"].include?(price_per)
    price_per.to_sym
  elsif ["person", "activity", "kg", "1/2 hour", "4 hours", "8 hours", "24 hours"].include?(price_per)
    # Custom unit types
    :custom
  else
    nil
  ...
  unless unit_type.nil?
    @listing.update_attribute(:unit_type, unit_type)
    # For custom unit types, clear both translation keys to use default translations
    if unit_type == :custom
      @listing.update_attribute(:unit_tr_key, nil)
      @listing.update_attribute(:unit_selector_tr_key, nil)
    end
  end
end

Given /^the visitor price of that listing is (\d+)\.(\d+) (EUR|USD)(?: per (.*?))?$/ do |price, price_decimal, currency, price_per|
  @listing.update_attribute(:visitor_price, Money.new(price.to_i * 100 + price_decimal.to_i, currency))
  @listing.update_attribute(:visitor_price_currency, currency)
end
...
Given /^the listing has Activity Types custom field with "(.*?)" selected$/ do |option_title|
  # Find or create the Activity Types custom field
  activity_types_field = CustomField.joins(:names)
                                   .where(community: @current_community, 
                                          custom_field_names: { value: 'Activity Types', locale: 'en' })
                                   .first
  
  unless activity_types_field
    # Create Activity Types checkbox field if it doesn't exist
    @category = @listing.category || find_category_by_name("Services")
    activity_types_field = FactoryBot.build(:custom_checkbox_field, 
                                           :community => @current_community, 
                                           :names => [CustomFieldName.create(:value => "Activity Types", :locale => "en")])
    activity_types_field.category_custom_fields << FactoryBot.build(:category_custom_field, 
                                                                    :category => @category, 
                                                                    :custom_field => activity_types_field)
    
    # Add standard activity type options
    ["Tours & Guides", "Experiences", "Classes & Lessons", "Equipment/Gear"].each do |option_name|
      title = CustomFieldOptionTitle.create(:value => option_name, :locale => "en")
      option = FactoryBot.build(:custom_field_option, :titles => [title])
      activity_types_field.options << option
    end
    
    activity_types_field.save!
  end
  
  # Find the specific option and create the selection
  option = activity_types_field.options.joins(:titles)
                               .where(custom_field_option_titles: { value: option_title, locale: 'en' })
                               .first
  
  if option
    value = FactoryBot.build(:checkbox_field_value, :listing => @listing, :question => activity_types_field)
    selection = CustomFieldOptionSelection.create!(:custom_field_value => value, :custom_field_option => option)
    value.custom_field_option_selections << selection
    value.save!
  end
