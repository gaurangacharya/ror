...
1.  Start the Sidekiq worker in a new console (open the project root folder):

    ```bash
    bundle exec sidekiq -q default -q paperclip -q mailers
    ```
...
1.  Start the Sidekiq worker:
    ...
    ```bash
    RAILS_ENV=production bundle exec sidekiq -q default -q paperclip -q mailers
    ```
